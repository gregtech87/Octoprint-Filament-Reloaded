...
class FilamentReloadedPlugin(octoprint.plugin.StartupPlugin,
	...
	def initialize(self):
		...
		self._printing = False         # For polling during prints
	...
	@property
	def poll_mode(self):
		# Poll_mode = True for polling, False for edge detect
		return self._settings.get_boolean(["poll_mode"])
	...
	def _setup_sensor(self):
		if self.sensor_enabled():
			...
			if self.poll_mode:
				self._logger.info("Using polling mode for filament detection.")
				if self._poll_thread is None:
					self._poll_running = True
					self._poll_thread = Thread(target=self._poll_sensor, daemon=True)
					self._poll_thread.start()
			else:
				self._logger.info("Using edge detection mode for filament detection.")
	...
	def _poll_sensor(self):
		self._logger.info("Starting filament polling thread.")
		while self._poll_running:
			try:
				if self._printing and self.sensor_enabled():
					pin_state = GPIO.input(self.pin)
					if pin_state != self.pin_value:
						self.pin_value = pin_state
						self._logger.info("Polling detected pin change. No filament = {}".format(self.no_filament()))
						if self.no_filament() and not self.triggered:
							self.triggered = 1
							self._handle_runout()
						elif not self.no_filament():
							self.triggered = 0
				sleep(self.checkrate / 1000)
			except Exception as e:
				self._logger.error("Polling thread error: {}".format(e))
				sleep(1)

	# Handle runout
	def _handle_runout(self):
		self._logger.info("Filament runout detected via polling")
		if self.pause_print:
			self._logger.info("Using pause option")
			self._printer.pause_print()
		if self.no_filament_gcode:
			self._logger.info("Using custom pause g-code")
	...
	def get_settings_defaults(self):
		return dict(
			...
			poll_mode=False,  # False = edge detect, True = polling
			no_filament_gcode='@pause Filament Runout',
			pause_print=False,
			prevent_print=True,
	...
	def on_event(self, event, payload):
		...
		if event in (
			...
			if self.sensor_enabled():
				...
				self.active = 1
				if not self.poll_mode:  # edge detect only if poll_mode=False
					GPIO.remove_event_detect(self.pin)
					GPIO.add_event_detect(
						self.pin, GPIO.BOTH,
						callback=self.sensor_callback,
